...
define ( "CAJA_GET_PRODUCT", "SELECT t.id,t.cod_prod,t.peso,p.nombre_producto,l.precio FROM temp_caja t, producto p, lista_precios l WHERE p.id_producto = t.cod_prod and p.id_producto = l.id_producto and cadenalogin = ?" );
...
define ( "PORTAL_SEL_MODULOS", "SELECT m.nombremodulo,m.Texto FROM rol_modulo r, modulo m WHERE r.id_modulo = m.id_modulo and r.id_rolempleado = ? ORDER BY m.nombremodulo" );
...
define ( "PORTAL_SEL_PRODUCTOS", "SELECT id_producto,nombre_producto FROM producto ORDER BY id_producto" );
define ( "PORTAL_SEL_LISTAPRECIOS", "SELECT l.id_producto, p.nombre_producto, l.precio, l.fechacambioprecio, l.precio_anterior FROM producto p, lista_precios l WHERE p.id_producto = l.id_producto" );

define ( "GRUPO_SEL_GRUPO", "SELECT nombre_carne FROM carne WHERE id_carne = ? ORDER BY id_carne" );
...
define ( "SUBGRP_SEL_SUBGRP", "SELECT nombre_ccarne FROM corte_carne WHERE id_ccarne = ? ORDER BY id_ccarne" );
...
define ( "PRODUCTO_INS_PRODUCTO", "INSERT INTO producto (id_producto,nombre_producto,id_ccarne,id_carne,unidad_medida)VALUES(?,?,?,?,?)" );
...
define ( "PRODUCTO_SEL_PRODCUTO", "SELECT nombre_producto,id_carne,id_ccarne,unidad_medida FROM producto WHERE id_producto = ?" );
...
define ( "LISTAPRECIO_INS_LISTAPRECIOS", "INSERT INTO lista_precios (precio,id_producto)VALUES (?,?)" );
define ( "LISTAPRECIO_DEL_LISTAPRECIO", "DELETE FROM lista_precios WHERE id_producto=?" );
define ( "LISTAPRECIO_UPD_PRODUCTO", "UPDATE lista_precios SET precio=?,precio_anterior=? WHERE id_producto=?" );
define ( "LISTAPRECIO_SEL_LISTAPRECIO", "SELECT precio,fechacambioprecio,precio_anterior FROM lista_precios WHERE id_producto = ?" );
define ( "LISTAPRECIO_SEL_PRODUCTOS", "SELECT id_producto,nombre_producto FROM producto ORDER BY nombre_producto" );
