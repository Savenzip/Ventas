...
define ( "PORTAL_SEL_MODULOS", "SELECT nombremodulo,Texto FROM modulo WHERE nivel = ? order by nombremodulo" );
...
define ( "LISTAPRECIO_INS_LISTAPRECIOS", "INSERT INTO lista_precios (id_lprecio,precio,fechacambioprecio,precio_anterior,id_procucto)values(?,?,now(),?,?)" ); // *
define ( "LISTAPRECIO_DEL_LISTAPRECIO", "DELETE from lista_precios where id_lprecios=?" );
define ( "LISTAPRECIO_UPD_PRODUCTO", "UPDATE lista_precios set precio=?,fechacambioprecio=now(),precio_anterior=? from lista_precios where id_lprecios=?" ); // *
define ( "LISTAPRECIO_SEL_LISTAPRECIO", "SELECT precio,fechacambioprecio,precio_anterior from lista precio where id_lprecios=?" ); // *
